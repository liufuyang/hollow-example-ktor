...
public class GcsIndexer implements Indexer {
  ...
  public List<Long> getSnapshotVersions() {
    try {
      ...
      final Blob indexBlob =
          storage.get(
              BlobId.of(bucketName, indexFilePath),
      ...
      LOG.warn(
          "Failed to read snapshot index at {}. Defaulting to empty list", getIndexFilePath(true));
    }
  ...
  @Override
  public void purge(final Collection<Long> versionsToRemove) {
    final ArrayList<Long> currentVersions = new ArrayList<>(getSnapshotVersions());
    currentVersions.removeAll(versionsToRemove);

    // clean up index file by removing all versions that is less than the min version of versionsToRemove
    long minVersionToRemove = versionsToRemove.stream().mapToLong(v -> v).max().orElse(0);
    List<Long> remainingVersions =
        currentVersions.stream()
            .filter(ver -> ver > minVersionToRemove)
            .collect(Collectors.toList());
    writeIndexContents(remainingVersions);
  }
