...
public class GcsPublisher implements HollowProducer.Publisher {
  ...
  private final GcsBlobCleaner cleaner;

  public GcsPublisher(final ApplicationConfig config, final Indexer indexer) {
    this(config, indexer, StorageOptions.getDefaultInstance().getService());
  }

  public GcsPublisher(
      final ApplicationConfig config, final Indexer indexer, final Storage storage) {
    ...
    this.cleaner = new GcsBlobCleaner(config, indexer);
  ...
  public void publish(final HollowProducer.Blob blob) {
    ...
    final long blobVersion =
        blob.getType().equals(HollowProducer.Blob.Type.SNAPSHOT)
            ? blob.getToVersion()
            : blob.getFromVersion();
    final String objectName = Util.getGcsObjectName(namespace, blob.getType().prefix, blobVersion);
    final BlobInfo blobInfo =
        BlobInfo.newBuilder(bucketName, objectName)
            .setMetadata(
                ImmutableMap.of(
                    "from_state",
                    String.valueOf(blob.getFromVersion()),
                    "to_state",
                    String.valueOf(blob.getToVersion())))
  ...
  public GcsBlobCleaner getCleaner() {
    return cleaner;
  }
