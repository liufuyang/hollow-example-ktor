...
public class RepositoryProducer extends AbstractProducer {
  ...
  public RepositoryProducer(
      final GcsAnnouncer announcer,
      final ApplicationConfig config,
      final GcsPublisher publisher,
    ...
    this.producerBuilder =
        HollowProducer.withPublisher(publisher)
    ...
    LOG.debug(
        "Starting a Hollow producer as primary: {} with {} states between snapshots",
        true,
  ...
  public void runCycle() {
    ...
    if (!updatedEntities.isEmpty() || !deletedEntities.isEmpty()) {
      LOG.debug(
          "Running producer cycle updating {} entities removing {}",
          updatedEntities.size(),
          deletedEntities.size());

      final long newVersion =
          Math.max(
              getMaxUpdatedTimestamp(updatedEntities), getMaxUpdatedTimestamp(deletedEntities));
  ...
  private List<InfoEntity> getUpdatedEntitiesAfter(final long timestamp) {
    return db.values().stream()
        .filter(e -> e.getTimeUpdated() > timestamp)
