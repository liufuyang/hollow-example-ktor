 ...
 * <pre>{@code
 *     HashIndex<Status, K> uki = HashIndex.from(consumer, Status.class)
 *         .usingBean(k);
 *     Stream<Status> results = uki.findMatches(k);
 * }</pre>
 * where {@code K} is a class declaring key field paths members, annotated with
 * {@link com.netflix.hollow.api.consumer.index.FieldPath}, and {@code k} is an instance of
 * {@code K} that is the query to find the matching {@code Status} objects.
 */
@Deprecated
@SuppressWarnings("all")
public class InfoEntityAPIHashIndex extends AbstractHollowHashIndex<InfoEntityAPI> {

    public InfoEntityAPIHashIndex(HollowConsumer consumer, String queryType, String selectFieldPath, String... matchFieldPaths) {
        super(consumer, true, queryType, selectFieldPath, matchFieldPaths);
    }

    public InfoEntityAPIHashIndex(HollowConsumer consumer, boolean isListenToDataRefresh, String queryType, String selectFieldPath, String... matchFieldPaths) {
    ...
    public Iterable<InfoEntity> findInfoEntityMatches(Object... keys) {
        HollowHashIndexResult matches = idx.findMatches(keys);
        if(matches == null) return Collections.emptySet();

        return new AbstractHollowOrdinalIterable<InfoEntity>(matches.iterator()) {
            public InfoEntity getData(int ordinal) {
